      ...
      <div style={{ display: 'flex', minHeight: '100vh' }}>
        ...
        <main style={{ flex: 1, minWidth: 0, padding: '44px 24px 32px 28px', boxSizing: 'border-box' }}>
          <header style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24, marginBottom: 34 }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#9b8f81', fontSize: 16 }}>
                ...
                <span>Ejecución</span>
                <BreadcrumbArrow />
                <span style={{ color: '#bb6a00', fontWeight: 700 }}>Gestión de Clientes</span>
              </div>
              <h1 style={{ margin: '8px 0 0', fontSize: 22, lineHeight: 1.1, fontWeight: 400, color: '#111111', fontFamily: 'Georgia, Times New Roman, serif' }}>Gestión de Clientes</h1>
            </div>
          ...
          <section style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 20, marginBottom: 34 }}>
            <MetricCard label="Total clientes" value={clients.length.toLocaleString()} detail="+12% este mes" accent />
            <MetricCard label="Vehículos" value="3,592" detail="Registrados en total" />
            <MetricCard label="Servicios mes" value="8,421" detail="En ejecución" />
          </section>
          ...
          <section style={{ background: colors.surface, borderRadius: 28, border: `1px solid ${colors.border}`, boxShadow: '0 10px 22px rgba(18, 39, 52, 0.05)', overflow: 'hidden' }}>
            ...
            <div style={{ background: '#edf3fa' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['CLIENT NAME', 'VEHICLES', 'TOTAL SERVICES', 'STATUS', 'ACTIONS'].map((label) => (
                ...
                <tbody>
                  {clients.map((row, index) => (
                    <tr key={row.id} style={{ background: index % 2 === 1 ? '#f0f1f3' : '#f8fbff' }}>
                      ...
                      <td style={{ padding: '18px 20px', color: '#111111', fontSize: 16, textAlign: 'center' }}>{row.vehicles}</td>
                      <td style={{ padding: '18px 20px', color: '#111111', fontSize: 16, textAlign: 'center' }}>{row.services}</td>
                      <td style={{ padding: '18px 20px', textAlign: 'center', color: '#111111', fontSize: 16 }}>
                      ...
                      <td style={{ padding: '18px 20px', textAlign: 'center' }}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10 }}>
                          <EditActionButton to={`/clients/${row.id}/edit`} />
                          <ViewActionButton to={`/clients/${row.id}`} />
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '28px 24px 24px', background: '#e8f1fb' }}>
              <div style={{ color: '#111111', fontSize: 16 }}>Mostrando 1–10 de 1,284 clientes</div>
